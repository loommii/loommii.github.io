...
1. 垃圾回收这道里的垃圾指的是什么？为什么要回收？
在程序运行的时候，进程会在堆内存申请内存空间。但是函数退出的时候栈空间会销毁。因此这一段堆内存空间没有任何的指针指向它（没有被引用）。因此会造成内存的浪费（内存泄露）。这一段的没有被引用的内存就是"垃圾"。，程序员不再需要手动管理内存的分配和释放，从而减少了由于内存管理不当引起的内存泄漏或悬空指针等问题。
...
2. 常见的GC有哪几种
     ...
     - 分代回收
     - 引用计数
3. 简述常见的GC如何实现，和他们的各自优缺点。
    - 手动垃圾回收：
        - 实现：程序员自行调用函数销毁
        - 优点：程序员对程序内存的掌控自由。
        - 缺点：容易出现提前释放（悬空指针）、忘记释放（内存泄露)等问题。
    - 标记-清扫法：
        - 实现：标记从根节点（栈内存、数据段）能追踪到的数据为游离数据，其他未标记的数据就是垃圾数据，将对垃圾数据进行回收
        - 优点：实现相对简单，
        - 缺点：容易造成内存碎片化
    - 标记-整理法：
        - 实现：
            - 标记阶段与`标记-清扫算法`一致。
            - 整理阶段：
                - 移动整理法：整理移动有用的数据，使有用的数据尽可能紧的放在内存里。
                    - 优点：解决了内存碎片化的问题
    ...
    - 引用计数
        - 实现：每次对对象应用都会更新对象的引用计数，当引用计数为0就回收该空间
        - 优点：可以及时回收垃圾内存
...
- 弱三色不变式：允许黑到白的情况，但是这个白色需要被灰色引用
![84ad5e2babcccd8c8b5a1c5c7d3b1260.png](./84ad5e2babcccd8c8b5a1c5c7d3b1260.png)

写屏障

- 插入写屏障
  - 滿足`强三色不变式`条件（关注白对象的写入操作）
    - 在增量式垃圾回收中,若出现白对象指向黑色则将白对象改为灰，或者把黑色改为灰。
    - 例子：Gc和用户代码交替的期间，假如new了一段新的空间，（黑->白）。则这个new的新空间直接标记为灰。（黑->灰）或将黑改为灰（灰->白）
- 删除写屏障
  - 滿足`弱三色不变式`条件 （关注对白对象路径破坏行为）
    - 在增量式垃圾回收中,若出现删除灰->白的引用时，则将白对象改为灰
    - 例子：Gc和用户代码交替的期间，若有灰对象 -> 白对象，程序将灰对象 = nil。着需要将白对象改为灰

读屏障
    解决标记-复制GC里面，在复制过程中。回收器将From改为To的过程中。如果要读取From要检查一下To区里面是不是已经复制了。如果没复制过来都都去To
...
> 作者: loommii  
> URL: https://loommii.github.io/zh-cn/posts/a_brief_discussion_on_go_gc_garbage_collection/  
