...
Goroutine  (go /ruːˈtiːn/  谐音 Go 如 听) 经 Golang 优化后的特越「协程」

### G限制

语言上无任何的限制,但是理论上会受到机器的内存限制,每个G创建时会占用4KB左右的内存空间

注：Goroutine 创建所需的 2-4KB 是需要连续的内存块。
...
>
>这个限制控制的是操作系统线程的数量，而不是 goroutine 的数量。只有当 goroutine 准备运行，但所依现有线程都因系统调用、cgo 调用被阻塞，或者由于使用了 runtime.LockOSThread 被锁定给其他 goroutine 时，Go 程序才会创建一个新的线程。
>
...
> 作者: loommii  
> URL: https://loommii.github.io/zh-cn/posts/go_gpm_quantity_limit/  
