...
![腾讯云SMS](image.png)
2. 隐藏请求参数
以营销活动为例，URL中的常见参数可能包括活动ID等信息。如果我们不希望这些ID被随意修改，可以在参数中添加对应的活动ID KEY，这样只有当ID和KEY匹配时，用户才能进入相应的活动页面。然而，添加了KEY后，原本的URL会变得更加冗长。短链服务可以帮助隐藏这些请求参数，保持URL简洁且安全。
...
虽然RFC3986标准允许一些特殊字符，但有些特殊字符可能会对URL解析、传输或存储造成问题。例如，字符如 &, ?, =, # 等在查询参数或路径中有特定含义，因此它们可能引起冲突或产生解析错误。

> 为了避免这些潜在的麻烦，特别是需要将复杂的查询参数或密钥编码为 URL 友好的格式时，我们通常会选择 Base62 编码，即只使用字母（大小写）和数字的组合。
>
### 短链KEY的长度选择
...
>5位的存储极限已经到达916,132,832,这个数量已经非常大，足以支撑大多数应用场景。然而，如果你希望能够进一步减少生成字符时的冲突、长远规划，选择 6 位 作为短链 KEY 长度会是一个不错的选择。
...
- 实现容易：随机生成短链 KEY 的实现相对简单，可以通过生成随机数并映射到 Base62 字符集，快速获得短链接标识符。大多数编程语言都提供了简单的随机数生成函数，操作方便。

缺点：

- 冲突的概率随着存量数据增多而增加
- 相同的长URL会生成不同的短链 KEY，可能导至浪费
...
- 解决了相同的长 URL 会生成不同短链的问题：哈希算法能够确保相同的长 URL 每次生成相同的短链 KEY，从而避免存储冗余。

缺点：

- 哈希冲突的可能性：虽然哈希函数可以生成固定长度的短链，但不同的长 URL 可能会产生相同的哈希值，从而产生哈希冲突。
![哈希冲突](image-1.png)
...
- 单机环境下不存在冲突的可能：由于计数器是自增的，每个短链 KEY 都是唯一的，因此不会产生冲突。

缺点:

- 生成的短链 KEY 存在明显的规律：由于短链 KEY 基于计数器生成，因此 KEY 存在明显的递增规律，容易被推测或猜测。
- 分布式环境下，存在冲突的可能：在分布式环境中，如果多个节点使用相同的计数器，可能会导致短链 KEY 冲突。
- 相同的长 URL 会生成不同的短链 KEY，可能导至浪费：每次生成短链时，相同的长 URL 会得到不同的短链 KEY。

解决: 可以通过计数倒置、乱序字典、哈希计算等方式优化生成逻辑，减少规律性和冲突。
...
- 计数倒置：为了打破短链生成中的规律性，可在计数器自增的基础上对生成的数字进行倒置处理，使每次数字变化幅度增大，生成的短链 KEY 更加多样化。
...
> 作者: loommii  
> URL: https://loommii.github.io/zh-cn/posts/service_design_how_to_design_a_url_shortening_service/  
