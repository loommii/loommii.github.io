...
In the current Go 1.16 version (in fact, only very early versions were different), the `int` type occupies 4 bytes on a 32-bit machine and 8 bytes on a 64-bit machine. So, on a 32-bit machine, the memory usage and layout of `int32` and `int` are exactly the same. However, Go does not perform implicit type conversion; `int` and `int32` are different types, so 2) in the problem above will not compile.
...
> Author: loommii  
> URL: https://loommii.github.io/en/go_practise/go_daily_question_069/  
